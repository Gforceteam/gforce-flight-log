...
story.extend(
    bullets(
        [
            "<b>GitHub</b> — sign up at <font face='Courier'>https://github.com/join</font>. After transfer, the repo lives under <font face='Courier'>Gforceteam</font> (use that org/user in every github.com URL and <font face='Courier'>gh</font> command below; Pages stays <font face='Courier'>https://gforceteam.github.io/...</font>).",
...
story.extend(
    bullets(
        [
            ...
            "New repo URL will be <font face='Courier'>https://github.com/Gforceteam/gforce-flight-log</font>.",
        ]
    )
)
...
story.append(
    prompt(
        ...
        "git ls-remote https://github.com/Gforceteam/gforce-flight-log.git\n\n"
...
story.append(
    prompt(
        ...
        "gh repo clone Gforceteam/gforce-flight-log\n"
        "cd gforce-flight-log\n\n"
        "2) Show me:\n"
        "- git remote -v\n"
        "- git branch --show-current\n"
        "- git log --oneline -5\n\n"
        "3) Confirm the remote points to Gforceteam/gforce-flight-log.\n"
...
story.append(
    prompt(
        ...
        "2) Ask me to pick YOUR_FLY_APP_NAME:\n"
        "   - gforce-api (if available), OR\n"
        "   - gforce-api-nz (if the name is already taken).\n\n"
        "3) Run:\n"
        "fly launch --no-deploy --copy-config --name YOUR_FLY_APP_NAME --region nrt --org personal\n\n"
        "4) Ask me to paste each secret value one at a time. Do not echo or log any value. Set each using:\n"
        "fly secrets set SECRET_NAME=pasted_value --app YOUR_FLY_APP_NAME --stage\n"
        "   (Run once per secret; replace SECRET_NAME and pasted_value with each name/value from the list.)\n"
        ...
        "- ALLOWED_ORIGINS=https://gforceteam.github.io\n\n"
        "5) Deploy:\n"
        "fly deploy --app YOUR_FLY_APP_NAME\n\n"
        "6) Smoke test:\n"
        "curl -s https://YOUR_FLY_APP_NAME.fly.dev/api/public/pilots | head -c 500\n"
        "Expected: JSON array of pilots.\n\n"
        "7) Print fly status --app YOUR_FLY_APP_NAME and confirm one machine is running."
    )
)
...
story.append(
    prompt(
        ...
        "3) Import backup:\n"
        "cat DUMP_FILE_PATH | turso db shell gforce-production\n\n"
        ...
        "fly secrets set TURSO_URL=PASTE_NEW_URL TURSO_AUTH_TOKEN=PASTE_NEW_TOKEN --app YOUR_FLY_APP_NAME\n\n"
        "7) Wait ~20 seconds then smoke test:\n"
        "curl -s https://YOUR_FLY_APP_NAME.fly.dev/api/public/pilots\n"
...
story.append(
    prompt(
        ...
        "- brookewhatnall.github.io -> gforceteam.github.io (GitHub Pages hostname; keep lowercase)\n"
        "- gforce-api.fly.dev -> YOUR_FLY_APP_NAME.fly.dev (only if YOUR_FLY_APP_NAME changed in Step 8)\n"
        ...
        "Then run:\n"
        'gh secret set FLY_API_TOKEN --body "PASTE_TOKEN_HERE" --repo Gforceteam/gforce-flight-log\n\n'
...
story.extend(
    bullets(
        [
            "Open: <font face='Courier'>https://gforceteam.github.io/gforce-flight-log/</font> on a phone.",
            ...
            "Claude should tail <font face='Courier'>fly logs --app YOUR_FLY_APP_NAME</font> the whole time.",
...
story.append(
    prompt(
        ...
        "curl -s https://gforceteam.github.io/gforce-flight-log/version.json\n\n"
